...
IND_TEST = [str(i).zfill(2) for i in [3,6,7,10,13,16,19,22,25,28,32]]
IND_EVAL = [str(i).zfill(2) for i in [2,4,8,11,14,17,20,23,27,30]]
IND_TRAIN = [str(i).zfill(2) for i in [1,5,9,12,15,18,21,24,26,29,31]]
...
extract_ind_data(path, min_peds=1, obs_len=25, pred_len=100, step=1, typ='test', original_rate= 0.04, resample_rate=0.1, num_workers=16)
extract_ind_data(path, min_peds=1, obs_len=25, pred_len=100, step=1, typ='eval', original_rate= 0.04, resample_rate=0.1, num_workers=16)
extract_ind_data(path, min_peds=1, obs_len=25, pred_len=100, step=1, typ='train', original_rate= 0.04, resample_rate=0.1, num_workers=16)
sys.exit() 