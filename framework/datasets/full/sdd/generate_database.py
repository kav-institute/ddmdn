...
def transform_segmentation(seg_map):
    ...
    new_seg_map[seg_map == SDD_SEG_DICT['blocking']] = TARGET_SEG_DICT['blocking']
    new_seg_map[seg_map == SDD_SEG_DICT['terrain']] = TARGET_SEG_DICT['terrain']
    new_seg_map[seg_map == SDD_SEG_DICT['road']] = TARGET_SEG_DICT['road']
    new_seg_map[seg_map == SDD_SEG_DICT['sidewalk']] = TARGET_SEG_DICT['sidewalk']
    new_seg_map[seg_map == SDD_SEG_DICT['bikelane']] = TARGET_SEG_DICT['bikelane']
    new_seg_map[seg_map == SDD_SEG_DICT['crosswalk']] = TARGET_SEG_DICT['crosswalk']
    new_seg_map[seg_map == SDD_SEG_DICT['vegetation']] = TARGET_SEG_DICT['vegetation']
...
class Sample_Generator(Dataset):
    ...
    def __init__(self, path, dataset, typ, obs_len=None, future_len=None, sample_rate=None, norm=False, augment=False, num_workers=8):
        ...
        self.seg_maps = {}
        self.scales = json.load(open(os.path.join(self.path, 'homography', 'scales.json')))
        for seq in list(raw_data.keys()):
            
            sm = cv2.cvtColor(cv2.imread(os.path.join(self.path, 'masks', f'{seq}_map.ppm'), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    ...
    def generate_ego_data(self, all_agents, other_data, adx, src):
        ...
        occupancy_grid = Occupancy_Grid_SDD(seg_map=copy.deepcopy(self.seg_maps[src]), scale=self.scales[src], radius=GRID_RADIUS, grid_size_px=GRID_SIZE_PX)
...
data_generator = Sample_Generator(path=path, dataset='sdd', typ='test', obs_len=10, future_len=40, sample_rate=0.1, norm=False, augment=False, num_workers=16)
data_generator = Sample_Generator(path=path, dataset='sdd', typ='train', obs_len=10, future_len=40, sample_rate=0.1, norm=False, augment=True, num_workers=16)
data_generator = Sample_Generator(path=path, dataset='sdd', typ='eval', obs_len=10, future_len=40, sample_rate=0.1, norm=False, augment=False, num_workers=16)
sys.exit()